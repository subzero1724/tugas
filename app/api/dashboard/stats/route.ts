...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Gagal mengambil data faktur",
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.items || body.items.length === 0) {
      return NextResponse.json({ success: false, error: "Faktur harus memiliki setidaknya satu item barang." }, { status: 400 })
    }

    for (const item of body.items) {
      if (!item.product_code || !item.product_name || !item.quantity || !item.unit_price) {
        return NextResponse.json({ success: false, error: `Data item tidak lengkap: ${item.product_name || 'item tanpa nama'}.` }, { status: 400 })
      }
      if (item.quantity <= 0) {
        return NextResponse.json({ success: false, error: `Jumlah (qty) untuk item ${item.product_name} harus lebih dari 0.` }, { status: 400 })
      }
      if (item.unit_price <= 0) {
        return NextResponse.json({ success: false, error: `Harga untuk item ${item.product_name} harus lebih dari 0.` }, { status: 400 })
      }
    }
    // --- Akhir Validasi ---
